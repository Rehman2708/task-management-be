...
async function enrichNoteComments(comments: INoteComment[], userCache: any) {
...
async function enrichNote(note: INote, userCache: any) {
  if (!note) return note;
...
router.get("/:ownerUserId", async (req, res) => {
  ...
  try {
    ...
    const enrichedNotes = await Promise.all(
      notes.map((note) => enrichNote(note, userCache))
    );
...
router.post("/:id/comment", async (req, res) => {
  ...
  try {
    ...
    setImmediate(async () => {
      try {
        const enrichedComments = await enrichNoteComments(
          [newComment],
          userCache
        );
