export const NotificationMessages = {
    Task: {
        ...
        Created: (props) => {
            const variants = [
                `🎯✨ ${props.creatorName} created "${props.taskTitle}" ${props.forYou ? "just for you—go crush it!" : ""}`,
    ...
    List: {
        ...
        Pinned: (props) => {
            const variants = [
                `📌✨ ${props.ownerName} ${props.pinned ? "pinned" : "unpinned"} "${props.listTitle}".`,
                `💡🚀 Heads-up! "${props.listTitle}" ${props.pinned ? "pinned" : "unpinned"} by ${props.ownerName}.`,
                `🎯🌟 ${props.listTitle} is now ${props.pinned ? "pinned" : "unpinned"} thanks to ${props.ownerName}.`,
                `⚡💜 Update: ${props.ownerName} ${props.pinned ? "pinned" : "unpinned"} the list "${props.listTitle}".`,
                `✨📌 "${props.listTitle}" ${props.pinned ? "pinned" : "unpinned"} by ${props.ownerName}.`,
            ];
            const body = variants[Math.floor(Math.random() * variants.length)];
        ...
        ItemStatus: (props) => {
            const variants = [
                `✅✨ ${props.ownerName} marked an item in "${props.listTitle}" as ${props.completed ? "done" : "not done yet"}.`,
                `⚡📌 "${props.listTitle}" update: item marked ${props.completed ? "complete" : "incomplete"} by ${props.ownerName}.`,
                `🎯💡 ${props.ownerName} just updated an item in "${props.listTitle}" to ${props.completed ? "done" : "not done yet"}.`,
                `💜🚀 Status alert: ${props.ownerName} marked an item in "${props.listTitle}" as ${props.completed ? "done" : "not done yet"}.`,
                `🌟📝 Item in "${props.listTitle}" marked ${props.completed ? "complete" : "pending"} by ${props.ownerName}.`,
            ];
            const body = variants[Math.floor(Math.random() * variants.length)];
    ...
    Note: {
        ...
        Pinned: (props) => {
            const variants = [
                `📌✨ ${props.ownerName} ${props.pinned ? "pinned" : "unpinned"} "${props.noteTitle}".`,
                `💡🚀 Heads-up! "${props.noteTitle}" ${props.pinned ? "pinned" : "unpinned"} by ${props.ownerName}.`,
                `🎯🌟 "${props.noteTitle}" is now ${props.pinned ? "pinned" : "unpinned"} thanks to ${props.ownerName}.`,
                `⚡💜 Update: ${props.ownerName} ${props.pinned ? "pinned" : "unpinned"} the note "${props.noteTitle}".`,
                `✨📌 "${props.noteTitle}" ${props.pinned ? "pinned" : "unpinned"} by ${props.ownerName}.`,
            ];
            const body = variants[Math.floor(Math.random() * variants.length)];
            return { title: `${props.pinned ? "📌 Pinned" : "📍 Unpinned"}`, body };
        },
